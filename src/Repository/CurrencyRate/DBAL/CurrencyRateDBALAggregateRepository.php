...
class CurrencyRateDBALAggregateRepository extends AbstractCurrencyRateDBALRepository implements CurrencyRateAggregateRepositoryInterface
{
    ...
    public function getAllByDateTimeRangeWithStep(
        ...
        $whereDate = $to ? 'datetime BETWEEN :from AND :to' : 'datetime >= :from';
        ...
        FROM currency_rate
        WHERE base=:base AND quote=:quote AND {$whereDate}
        GROUP BY (
        ...
        if (null !== $to) {
            $params['to'] = $to->format(static::DATETIME_FORMAT);
        }

        try {
            $data = $this->connection->prepare($sql)->executeQuery($params)->fetchAllAssociative();
        } catch (\Throwable $e) {
